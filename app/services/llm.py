...
class LLMService:
    """
    Thin pluggable LLM adapter for local Ollama.
    """
    ...
    def complete(self, prompt: str, system_prompt: str | None = None) -> LLMResult:
        if not self.enabled:
    ...
    def health(self) -> dict[str, Any]:
        ...
        except Exception as exc:  # pragma: no cover - operational network check
            return {
                "ok": False,
                "provider": "ollama",
    ...
    def _post_json(self, url: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(f"Ollama HTTP error {exc.code}: {response_text}") from exc
        except URLError as exc:
            raise RuntimeError(f"Cannot connect to Ollama at {self.base_url}: {exc.reason}") from exc
